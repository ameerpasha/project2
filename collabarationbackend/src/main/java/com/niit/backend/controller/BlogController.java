...
public class BlogController {
	...
	public ResponseEntity<Blog> getBlog(@PathVariable("id") int blog_id){
		logger.debug("calling method delete blog for blog id: "+ blog_id);
		Blog blog=blogDao.get(blog_id);
		if(blog==null)
		{
			logger.debug("user already exists with is:"+ blog_id);
			blog.setErrormessage("blog saved with id"+blog_id);
			return new ResponseEntity<Blog>(blog,HttpStatus.NOT_FOUND);
		}
		
	logger.debug("blog exist with id:"+blog_id);
	return new ResponseEntity<Blog>(blog,HttpStatus.OK);
